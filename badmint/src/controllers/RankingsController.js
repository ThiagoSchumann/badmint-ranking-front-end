...
async function getRankingQuery(rankingId, categoryId) {
  // example for now
  // api/ranking-query&cat_id=57&ryear=2022&week=40&page_size=25&page_no=1
  console.log(`get ranking query path: api/ranking-query?ranking=${rankingId}&category=${categoryId}&athlete_name=lucas&athlete_age=15`)
  const response = await ApiAdapter.get(`api/ranking-query?ranking=${rankingId}&category=${categoryId}&athlete_name=lucas&athlete_age=15`)
  return response.data;
}
